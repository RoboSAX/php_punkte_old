		...
		<tbody>
			<?php
				...
				while ($db_spiel = $db_spiele -> fetch(PDO::FETCH_ASSOC)) {
					...
					echo "<td>".($db_spiel["fixiert"] ? "ja" : "nein")."</td>";
					echo "<td>".substr($db_spiel["zeit"],0,5)."</td>";
					echo "<td>".($db_spiel["punktegeaendert"] ? "ja" : "nein")."</td>";
					echo "<td>".($db_spiel["zeitgeaendert"] ? "ja" : "nein")."</td></tr>";
				}
			?>
		</tbody>
	</table>
	<input type="submit" name="addspiel" value="Spiel hinzufügen">
